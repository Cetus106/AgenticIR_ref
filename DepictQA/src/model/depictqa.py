...
class DepictQA(nn.Module):
    ...
    def tokenize_query(self, queries, task_type):
        if not queries or len(queries) == 0:
            raise ValueError("Queries list is empty. Please provide at least one query.")
        for i, query in enumerate(queries):
            if not query or not isinstance(query, str) or len(query.strip()) == 0:
                raise ValueError(f"Query at index {i} is empty or invalid. Please provide valid text.")
        if self.unique_tag and "quality_compare" in task_type:
            eov_final = eov["B"]
        else:
            eov_final = eov["img"]
        ...
        try:
            ...
            raise ValueError("Failed to tokenize input text. Please check the text content.")

        if input_tokens.input_ids.nelement() == 0:
            raise ValueError("Tokenization resulted in empty sequence. Please check the tokenizer and input text.")

        logging.debug(f"Tokenized texts: {texts}")
        logging.debug(f"Tokenized input_ids shape: {input_tokens.input_ids.shape}")
        logging.debug(f"Tokenized input_ids: {input_tokens.input_ids}")
    ...
    def get_generate_embs(self, inputs):
        ...
        if input_ids.shape[1] == 0:
            raise ValueError("Tokenized input sequence is empty. Please check your input queries.")
        ...
        if input_embs.shape[1] == 0:
            raise ValueError("Final embeddings have sequence length 0. This indicates an issue with input processing.")
    ...
    def generate(self, inputs):
        ...
        if input_embs.shape[1] == 0:
            raise ValueError("Final embeddings have sequence length 0. Aborting generation.")

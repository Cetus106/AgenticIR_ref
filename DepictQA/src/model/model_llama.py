...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if seq_length == 0:
            raise ValueError(
                "Sequence length cannot be zero. Please provide valid input. "
                ...
                f"query_embeds shape: {query_embeds.shape if query_embeds is not None else 'None'}"
            )
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # 这里是为了解决复现时可能是库不一致所致的bug
        if input_ids.shape[1] == 0 and inputs_embeds is not None:
            past_key_values = None
